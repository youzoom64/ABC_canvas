    ...
    if args.operation == "command-children":
        body = {
            **base_payload,
            ...
            "continueOnError": payload.get("continueOnError", payload.get("continue_on_error", True)) is not False,
            "parallel": payload.get("parallel", True) is not False,
            "maxParallel": int(payload.get("maxParallel", payload.get("max_parallel", 3)) or 3),
            "staggerMs": int(payload.get("staggerMs", payload.get("stagger_ms", 1000)) or 0),
        }
